...
public class SocketHandle implements Runnable {
    ...
    //get list user 
    //(id,nickname,isOnline,isPlaying)
    ...
    //(id,username,password,nickname,avatar,numberOfGame,numbarofWin,
    //  numberOfDraw,isOnline,isPlaying)
    ...
    public void run() {
        ...
        try {
            socketOfClient = new Socket("127.0.0.1", 8888);            // Gửi yêu cầu kết nối tới Server đang lắng nghe
            System.out.println("Kết nối thành công!");
            os = new BufferedWriter(new OutputStreamWriter(socketOfClient.getOutputStream()));// Tạo luồng đầu ra tại client (Gửi dữ liệu tới server)
            ...
            while (true) {
                ...
                if (messageSplit[0].equals("wrong-user")) {
                    ...
                    Client.loginView.showError("Tài khoản hoặc mật khẩu không chính xác");
                }
                ...
                if (messageSplit[0].equals("dupplicate-login")) {
                    ...
                    Client.closeView(Client.View.GAMENOTICE);
                    Client.openView(Client.View.LOGIN, messageSplit[1], messageSplit[2]);
                    Client.loginView.showError("Tài khoản đã đăng nhập ở nơi khác");
                }
                //Tài khoản đã bị banned
                if (messageSplit[0].equals("banned-user")) {
                    Client.closeView(Client.View.GAMENOTICE);
                    Client.openView(Client.View.LOGIN, messageSplit[1], messageSplit[2]);
                    Client.loginView.showError("Tài khoản đã bị ban");
                }

                //Xử lý nhận thông tin, chat từ toàn server
                if (messageSplit[0].equals("chat-server")) {
                    if (Client.homePageView != null) {
                        Client.homePageView.addMessage(messageSplit[1]);
                    }
                }
                ...
                if (messageSplit[0].equals("room-fully")) {
                    Client.closeAllViews();
                    Client.openView(Client.View.HOMEPAGE);
                    JOptionPane.showMessageDialog(Client.homePageView, "Phòng chơi đã đủ 2 người chơi");
                }
                // Xử lý không tìm thấy phòng trong chức năng vào phòng
                if (messageSplit[0].equals("room-not-found")) {
                    Client.closeAllViews();
                    Client.openView(Client.View.HOMEPAGE);
                    JOptionPane.showMessageDialog(Client.homePageView, "Không tìm thấy phòng");
                }
                // Xử lý phòng có mật khẩu sai
                if (messageSplit[0].equals("room-wrong-password")) {
                    Client.closeAllViews();
                    Client.openView(Client.View.HOMEPAGE);
                    JOptionPane.showMessageDialog(Client.homePageView, "Mật khẩu phòng sai");
                }
                ...
                if (messageSplit[0].equals("room-list")) {
                    ...
                    Client.roomListView.updateRoomList(rooms, passwords);
                }
                ...
                if (messageSplit[0].equals("go-to-room")) {
                    ...
                    if (Client.findRoomView != null) {
                        Client.findRoomView.showFindedRoom();
                        try {
                            Thread.sleep(3000);
                        } catch (InterruptedException ex) {
                            JOptionPane.showMessageDialog(Client.findRoomView, "Lỗi khi sleep thread");
                        }
                    } else if (Client.waitingRoomView != null) {
                        Client.waitingRoomView.showFindedCompetitor();
                        try {
                            Thread.sleep(3000);
                        } catch (InterruptedException ex) {
                            JOptionPane.showMessageDialog(Client.waitingRoomView, "Lỗi khi sleep thread");
                        }
                    }
                    Client.closeAllViews();
                    ...
                    Client.openView(Client.View.GAMECLIENT,
                             ...
                             isStart,
                             competitorIP);
                    Client.gameClientView.newgame();
                }
                //Tạo phòng và server trả về tên phòng
                if (messageSplit[0].equals("your-created-room")) {
                    Client.closeAllViews();
                    Client.openView(Client.View.WAITINGROOM);
                    Client.waitingRoomView.setRoomName(messageSplit[1]);
                    if (messageSplit.length == 3) {
                        Client.waitingRoomView.setRoomPassword("Mật khẩu phòng: " + messageSplit[2]);
                    }
                }
                ...
                if (messageSplit[0].equals("disagree-duel")) {
                    Client.closeAllViews();
                    Client.openView(Client.View.HOMEPAGE);
                    JOptionPane.showMessageDialog(Client.homePageView, "Đối thủ không đồng ý thách đấu");
                }
                //Xử lý đánh một nước trong ván chơi
                if (messageSplit[0].equals("caro")) {
                    Client.gameClientView.addCompetitorMove(messageSplit[1], messageSplit[2]);
                }
                if (messageSplit[0].equals("chat")) {
                    Client.gameClientView.addMessage(messageSplit[1]);
                }
                if (messageSplit[0].equals("draw-request")) {
                    Client.gameClientView.showDrawRequest();
                }

                if (messageSplit[0].equals("draw-refuse")) {
                    if (Client.gameNoticeView != null) {
                        Client.closeView(Client.View.GAMENOTICE);
                    }
                    Client.gameClientView.displayDrawRefuse();
                }

                if (messageSplit[0].equals("new-game")) {
                    System.out.println("New game");
                    Thread.sleep(4000);
                    Client.gameClientView.updateNumberOfGame();
                    Client.closeView(Client.View.GAMENOTICE);
                    Client.gameClientView.newgame();
                }
                if (messageSplit[0].equals("draw-game")) {
                    ...
                    Client.gameClientView.displayDrawGame();
                    Thread.sleep(4000);
                    Client.gameClientView.updateNumberOfGame();
                    Client.closeView(Client.View.GAMENOTICE);
                    Client.gameClientView.newgame();
                ...
                if (messageSplit[0].equals("competitor-time-out")) {
                    Client.gameClientView.increaseWinMatchToUser();
                ...
                if (messageSplit[0].equals("voice-message")) {
                    switch (messageSplit[1]) {
                        case "close-mic":
                            Client.gameClientView.addVoiceMessage("đã tắt mic");
                            break;
                        case "open-mic":
                            Client.gameClientView.addVoiceMessage("đã bật mic");
                            break;
                        case "close-speaker":
                            Client.gameClientView.addVoiceMessage("đã tắt âm thanh cuộc trò chuyện");
                            break;
                        case "open-speaker":
                            Client.gameClientView.addVoiceMessage("đã bật âm thanh cuộc trò chuyện");
                            break;
                    }
                }
                if (messageSplit[0].equals("left-room")) {
                    Client.gameClientView.stopTimer();
                    Client.closeAllViews();
                ...
                if (messageSplit[0].equals("banned-notice")) {
                    ...
                    JOptionPane.showMessageDialog(Client.loginView, messageSplit[1], "Bạn đã bị BAN", JOptionPane.WARNING_MESSAGE);
                }
                //Xử lý cảnh cáo
